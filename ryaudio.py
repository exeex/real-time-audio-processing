...
class RyAudio:
    ...
    def __init__(self, Fs=16000, TinSec=10):
        ...
        self.specgram = np.zeros([self.frameN, self.spBufferSize // 2])
    ...
    def getSound(self):
        ...
        while (self.gettingSound is True):  # 利用 self.gettingSound= False 來中止此無窮迴圈。
            ...
            #
            xFFT = np.fft.fft(x)[0:spBufferSize // 2]  # for x be real, a half range of fft is enough

            #
            # 訊號的功率頻譜(正實數值)， 如上，只保存 一半 的 頻率資訊
            #
            xP = np.absolute(xFFT * xFFT.conj())  # Power spectrum
            sumXp = xP.sum()
            ...
            # 頻率亂度 (entropy)
            self.entropy = -(xP * np.log(xP / sumXp)).sum() / sumXp
